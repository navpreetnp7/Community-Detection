...
from layers import GraphNeuralNet, InnerProduct
import torch
...
class GNN(nn.Module):
    ...
    def __init__(self, batch_size, nfeat, nhid, ndim, mu0, sigma0, fixed):
        ...
        self.gc1 = GraphNeuralNet(batch_size, nfeat, nhid, mu0, sigma0, scale=False)
        self.fixed = fixed
        if self.fixed:
            self.embeddings = GraphNeuralNet(batch_size, nhid, 2 * ndim, mu0, sigma0, scale=True)
            self.reconstructions = InnerProduct(2 * ndim)
        else:
            self.embeddings = GraphNeuralNet(batch_size, nhid, 4 * ndim, mu0, sigma0, scale=True)
